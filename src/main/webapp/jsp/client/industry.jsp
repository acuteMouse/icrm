...
//                            验证提示清空
                        ...
                        if (arr.length < 1) {
                            ...
                            $.messager.confirm('友情提示', '确定要删除吗？', function (r) {
                                if (!r) {
                                    ...
                                    $.ajax({
                                        url: 'customer/industry_delete.action',
                                        data: {'ids': ids},//发送数据
                                        type: "post",
                                        success: function () {
                                            $('#dg').datagrid('reload');
                ...
                pagination: true,
                url: 'customer/industry_getAll.action',
                idField: 'id',
                frozenColumns: [[{
                    field: 'checkbox',
                    width: 50,
                ...
                columns: [[
                    ...
                    {field: 'i_name', title: '名称', width: 100},
                ]],
//                设置分页初始值，可选项
                pageNumber: 1,
                pageSize: 15,
            ...
            $('#btn1').click(
                    function () {
                        if ($('#myform').form('validate')) {
                            $.ajax({
                                type: 'post',
                                url: flag == 'add' ? 'customer/industry_add.action' : 'customer/industry_update.action',
        ...
        function checkTypeIsExited() {
            var typeName = $('input[name="i_name"]').val();//获取填写的信息
//            alert(typeName);
//            异步验证是否可用
            $.ajax({
                url: "customer/industry_checkIndustyrIsExited.action",
                data: "industry.i_name=" + typeName,
                type: "post",
...
<table id="dg"></table>
<%--添加信息框--信息修改框--%>
<div id="dd" class="easyui-dialog" title="添加行业" modal=true
     style="width: 400px;"
    ...
    <form id="myform" method="post" style="text-align: center;">
        ...
        客户行业:
        <input class="easyui-validatebox" type="text" name="i_name" onmouseout="checkTypeIsExited()"
